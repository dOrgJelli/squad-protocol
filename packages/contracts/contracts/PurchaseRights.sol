...
contract PurchaseRights is Ownable, IRightsManager {
    /**
    ...
    function registerNFT(address nftAddress, uint256 nftId) external override onlyOwner returns (bool) {
        ...
        string memory name = string(abi.encodePacked("PurchaseRights", nft.name(), nftId));
    ...
    function price(address nftAddress, uint256 nftId) public view returns (address, uint256) {
        ...
        (address tokenAddress, uint256 amount) = squad.rightsParamsFor(nftAddress, nftId);
    ...
    function check(address nftAddress, uint256 nftId, address toCheck) external view returns (bool) {
